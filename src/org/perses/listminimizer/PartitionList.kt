...
class PartitionList<T : Any>(
  ...
  fun duplicateByRemovePartition(toBeRemoved: Partition<T>): PartitionList<T> {
    val builder = ImmutableList.builderWithExpectedSize<Partition<T>>(partitions.size - 1)
    var found = false
    partitions.forEach {
      if (it === toBeRemoved) {
        found = true
    ...
    return PartitionList(builder.build())
  }
