...
import org.perses.listminimizer.AbstractListInputMinimizerListener.NullListInputMinimizerListener
import org.perses.util.transformToImmutableList

abstract class AbstractListInputMinimizer<T : Any, PropertyPayload>(
  ...
  fun reduce(): ImmutableList<T> {
    try {
      arguments.listener.startReduction(originalInput, this)
      best = originalInput
      ...
      if (arguments.needToTestEmpty) {
        ...
        log { "Testing the empty input is disabled." }
      }
      reduceNonEmptyInput()
      return best.transformToImmutableList { it.element }
    } finally {
  ...
  data class Arguments<T : Any, PropertyPayload>(
    ...
    val listener: AbstractListInputMinimizerListener = NullListInputMinimizerListener,
  )
