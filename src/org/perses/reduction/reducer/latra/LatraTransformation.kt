...
open class LatraTransformation(
  ...
  protected val parserFacade: AbstractParserFacade
    get() = latraArguments.parserFacade
  ...
  private fun createPropertyTester(
    ...
    if (matches.isEmpty()) {
      ...
      return@IPropertyTester LMPropertyTestResult.Skipped(
    ...
    LMPropertyTestResult.Completed(
      result =
        if (testProperty(edit)) {
  ...
  companion object {
    private fun summarizeMatchInfo(matches: List<SparTreeHoleMatch>): String {
      ...
      return when {
        size == 0 -> error("Unreachable")
        size == 1 -> matches.single().printMatchInfo()
        size <= 5 -> matches.map { it.printMatchInfo() }.toString()
        else -> {
