...
object FineGritLatraReducerAnnotation :
  AbstractNonDeletionBasedReducer.NonDeletionBasedReducerAnnotation(
    shortName = "FineGritLatraReducer",
    description = "Perform fine-grit language-specific program transformations.",
  ...
  override fun create(reducerContext: ReducerContext): ImmutableList<AbstractSparTreeReducer> =
    ImmutableList.of(
      DefaultLatraReducer(
        ...
        language2transformationListMap = TransformationRegistry.findGritTransformations,
        enforceStrictlyMonotonicSizeDecrease = false,
      ),
    )
}
