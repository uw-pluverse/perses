...
class PersesGrammar(
  ...
  val flattenedAllRules: ImmutableList<AbstractPersesRuleDefAst> =
    ImmutableList
  ...
  fun getRuleNameHandleOrThrow(ruleName: String): RuleNameHandle =
    symbolTable.ruleNameRegistry.getOrThrow(ruleName)
  ...
  fun copyWithNewLexerRuleDefs(newLexerRules: LexerRuleList): PersesGrammar =
    PersesGrammar(
      grammarType,
      grammarName,
      channelDefinitions,
  ...
  fun getRuleDefinition(ruleNameHandle: RuleNameHandle): AbstractPersesRuleDefAst? =
    ruleNameToRuleMap[ruleNameHandle]
  ...
  fun isEquivalent(other: PersesGrammar): Boolean {
    ...
    for (ruleNameHandle in ruleNameToRuleMap.keys) {
      val thisBody = ruleNameToRuleMap[ruleNameHandle]!!.body
      val otherBody = other.ruleNameToRuleMap[ruleNameHandle]!!.body
      if (!thisBody.isEquivalent(otherBody)) {
        return false
      }
    }
    return true
