...
import org.perses.listminimizer.localexhaust.UniformLengthDeletionPatternSet.Companion.createOperationListWithGivenNumOfDeletes
import org.perses.listminimizer.localexhaust.UniformLengthDeletionPatternSet.Companion.createPatternsWithGivenNumOfDeletes

@RunWith(JUnit4::class)
class UniformLengthDeletionPatternSetTest {
  @Test
  fun test() {
  }

  @Test
  fun testGetInterestingPatternsDescending() {
    CachedUniformLengthDeletionPatternSets.getDeletionPatternSet(patternLength = 1).let {
      assertThat(it.interestingPatternsInDescendingOfNumOfDeletes).isEmpty()
    }

    CachedUniformLengthDeletionPatternSets.getDeletionPatternSet(patternLength = 2).let {
      assertThat(it.interestingPatternsInDescendingOfNumOfDeletes).hasSize(1)
    }

    CachedUniformLengthDeletionPatternSets.getDeletionPatternSet(patternLength = 3).let {
      val interesting = it.interestingPatternsInDescendingOfNumOfDeletes
      assertThat(interesting.map { pattern -> pattern.numOfDeletes }.distinct()).hasSize(2)
      assertThat(interesting.distinct()).hasSize(4)
    }

    CachedUniformLengthDeletionPatternSets.getDeletionPatternSet(patternLength = 4).let {
      val interesting = it.interestingPatternsInDescendingOfNumOfDeletes
      assertThat(interesting.map { pattern -> pattern.numOfDeletes }.distinct()).hasSize(3)
      assertThat(interesting.distinct()).hasSize(11)
    }
  }
  ...
  fun testGetDeletionPatterns() {
    CachedUniformLengthDeletionPatternSets
      .getDeletionPatternSet(
        ...
        assertThat(it.totalPatternCount).isEqualTo(16)
      }
    CachedUniformLengthDeletionPatternSets
      .getDeletionPatternSet(
        patternLength = 5,
      ).let {
        assertThat(it.totalPatternCount).isEqualTo(32)
      }
  }

  @Test
  fun testPatternSetWithPatternLengthOne() {
    val patternSet = UniformLengthDeletionPatternSet(patternLength = 1)
    assertThat(patternSet.totalPatternCount).isEqualTo(2)
  ...
  @Test
  fun testPatternSetWithPatternLengthTwo() {
    val patternSet = UniformLengthDeletionPatternSet(patternLength = 2)
    assertThat(patternSet.totalPatternCount).isEqualTo(4)
  ...
  @Test
  fun testCreatePatternsWithGivenNumOfDeletesWithPatternLengthTwoAndNumOfDeletesOne() {
    val patterns =
    ...
    assertThat(patterns[0].operations)
      .containsExactly(
        KEEP,
        DELETE,
      ).inOrder()
  ...
  fun testCreatePatternsWithGivenNumOfDeletes_total_3_deletes_2() {
    ...
    assertThat(patterns[1].operations)
      .containsExactly(
        DELETE,
        KEEP,
        DELETE,
