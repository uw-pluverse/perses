...
class ConcurrentStateBasedLineSlicerTest : AbstractReducerFunctionalTest() {
  ...
  fun testDelta1GrepBased() {
    test(
      ...
      sourceFile = "t.c",
      algorithmType = ConcurrentStateBasedLineSlicer.COMPOSITE_REDUCER,
      cmdCustomizer = {},
...
""".trim(),
  ...
  fun testStateAdvance() {
    val stateNull = ConcurrentLineSlicingState.create(granularity = 5, sequenceSize = 5)
    Truth.assertThat(stateNull).isNull()

    // "int a;"
    // "return 0;"
    // slicing with granularity = 1
    val state0 = ConcurrentLineSlicingState.create(granularity = 1, sequenceSize = 2)
  ...
  fun testStateAdvanceOnSuccess() {
    // given program [a, b, c, d, e], each represents a line.
    val state0 = ConcurrentLineSlicingState.create(2, 5)
