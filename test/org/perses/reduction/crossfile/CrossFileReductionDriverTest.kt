...
class CrossFileReductionDriverTest {
  // The property holds iff BOTH files still contain KEEP. The initial on-disk content (with DROP
  // lines) already passes, so the driver's sanity check succeeds.
  private val fixture =
  ...
  private val driver = fixture.createDriver(ImmutableList.of(DropLinesTransformation))
  ...
  @Test
  fun testDriverReducesTheWholeSetToFixpoint() {
    driver.reduce()

    // Both files have had their DROP lines stripped; the property (KEEP in both) is preserved, and
    // the result folder is the single source of truth.
