...
class MimirJavacDependencyAnalyzerDumpMain(
  ...
  private fun printDefUseInformation(
    ...
    analyzer: JavacDependencyAnalyzer,
  ): String {
    ...
    fun main(args: Array<String>) {
      val processor =
        CommandLineProcessor(
          ...
          programName = MimirJavacDependencyAnalyzerDumpMain::class.qualifiedName!!,
          args = args,
        )
      if (processor.process() == CommandLineProcessor.HelpRequestProcessingDecision.EXIT) {
        return
      }
      MimirJavacDependencyAnalyzerDumpMain(processor.cmd).run()
    }
  }
}
