...
if ! timeout -s 9 30 rustc -Z parse-only mutant.rs ; then 
  exit 1
fi	
...
if ! grep --quiet --fixed-strings "error: internal compiler error: compiler/rustc_typeck/src/collect/type_of.rs" "${OUTPUT}" ; then
  exit 1
fi
