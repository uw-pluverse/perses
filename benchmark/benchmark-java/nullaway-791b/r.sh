...
if [ -z "${GRADLE_DIR}" ]; then echo "Error: GRADLE_DIR is not set."; exit 1; fi
if [ -z "${JDK21_DIR}" ]; then echo "Error: JDK21_DIR is not set."; exit 1; fi
...
grep -qF 'An unhandled exception was thrown by the Error Prone static analysis plugin.' output.txt && grep -qF 'com.uber.nullaway.GenericsChecks.typeWithPreservedAnnotations' output.txt && echo "Found pattern." && exit 0
