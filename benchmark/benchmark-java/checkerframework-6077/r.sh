...
if [ -z "${CHECKER_3_36_0}" ]; then echo "Error: CHECKER_3_36_0 is not set."; exit 1; fi
...
"${JDK8_DIR}/bin/java" -jar "${CHECKER_3_36_0}/checker.jar" -J-da -processor org.checkerframework.checker.resourceleak.ResourceLeakChecker -AprintErrorStack \
  $(find . -name "*.java") > output.txt 2>&1
grep -E 'IndexMode\.java:[0-9]+: error: \[required.method.not.known\]' output.txt && echo "Found pattern." && exit 0
