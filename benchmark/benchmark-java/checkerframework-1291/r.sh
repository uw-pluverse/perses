...
if [ -z "${CHECKER_2_1_10}" ]; then echo "Error: CHECKER_2_1_10 is not set."; exit 1; fi
...
"${JDK8_DIR}/bin/java" -jar "${CHECKER_2_1_10}/checker.jar" -J-da -processor org.checkerframework.checker.nullness.NullnessRawnessChecker -AprintErrorStack \
  $(find . -name "*.java") > output.txt 2>&1
grep -E 'PureMethodInfo\.java:[0-9]+: error: \[return.type.incompatible\]' output.txt && echo "Found pattern." && exit 0
