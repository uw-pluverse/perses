...
if [ -z "${CHECKER_3_6_0}" ]; then echo "Error: CHECKER_3_6_0 is not set."; exit 1; fi
...
"${JDK8_DIR}/bin/java" -jar "${CHECKER_3_6_0}/checker.jar" -J-da -processor org.checkerframework.checker.nullness.NullnessChecker -AprintErrorStack \
  $(find . -name "*.java") > output.txt 2>&1
grep -E 'ImmutableIntList\.java:[0-9]+: error: \[contracts.conditional.postcondition.not.satisfied\]' output.txt && echo "Found pattern." && exit 0
