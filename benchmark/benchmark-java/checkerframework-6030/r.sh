...
if [ -z "${CHECKER_3_35_0}" ]; then echo "Error: CHECKER_3_35_0 is not set."; exit 1; fi
...
"${JDK8_DIR}/bin/java" -jar "${CHECKER_3_35_0}/checker.jar" -J-da -processor org.checkerframework.checker.resourceleak.ResourceLeakChecker -AprintErrorStack \
  $(find . -name "*.java") > output.txt 2>&1
grep -E 'unexpected Throwable \(NullPointerException\) while processing.*SSTableScanner\.java' output.txt && echo "Found pattern." && exit 0
