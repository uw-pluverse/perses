...
grep -qF 'An unhandled exception was thrown by the Error Prone static analysis plugin.' output.txt && grep -qF 'com.uber.nullaway.NullAway.safeInitByCalleeBefore' output.txt && echo "Found pattern." && exit 0
