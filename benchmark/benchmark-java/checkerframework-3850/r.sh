...
if [ -z "${CHECKER_3_7_1}" ]; then echo "Error: CHECKER_3_7_1 is not set."; exit 1; fi
...
"${JDK8_DIR}/bin/java" -jar "${CHECKER_3_7_1}/checker.jar" -J-da -processor org.checkerframework.checker.nullness.NullnessChecker -AprintErrorStack \
  $(find . -name "*.java") > output.txt 2>&1
grep -E 'error: InternalUtils.symbol: tree is null' output.txt && echo "Found pattern." && exit 0
