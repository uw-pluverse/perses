...
def parse_arguments():
    ...
    parser.add_argument("-c", "--comparison-mode", default=False, type=bool,
...
def csv_filename_generator(flag: str) -> str:
    """Generate a proper name for the snapshot"""
    ...
    if flag:
        return f'snapshot_{time}_{git_head_id}_{flag}'
    else:
        return f'snapshot_{time}_{git_head_id}'
...
def main():
    ...
    dir = arguments.input_directory
    output_flag = arguments.output_flag
    comparison_mode = arguments.comparison_mode

    benchmark_results_location = os.path.abspath(dir)
    snapshots_location = os.path.join(__location__, 'benchmark_snapshots')

    file_list = get_file_list(benchmark_results_location)
    config = extract_config(file_list[0])

    if comparison_mode:
        header_row = ['Bench', 'Query', 'Time', 'Token_remaining', 'Environment']
    else:
        header_row = ['Bench', 'Query', 'Time', 'Token_remaining']
    ...
    output_manager(snapshots_location, csv_rowlist, output_flag, config)
