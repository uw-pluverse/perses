...
readonly CORRECT_RUSTC_VERSION="nightly-2020-10-10"
...
if ! timeout -s 9 60 rustup run "${BUGGY_RUSTC_VERSION}" rustc -Z mir-opt-level=2 -o "${EXE_WRONG}" "${FILE}"; then
  exit 1
fi
...
if ! grep "How are you feeling" "${OUTPUT_CORRECT_1}" ; then
  exit 1
fi
