...
__floatsisf(int a) {
    ...
    rep_t sign = 0;
    if (a < 0) {
        sign = (1U << (23 + ((sizeof(rep_t)*8) - 23 - 1)));
        a = -a;
    }
    ...
    if (exponent <= 23) {
        ...
        result = (rep_t)a << shift ^ (1U << 23);
    } else {
        const int shift = exponent - 23;
        result = (rep_t)a >> shift ^ (1U << 23);
        rep_t round = (rep_t)a << ((sizeof(rep_t)*8) - shift);
        if (round > (1U << (23 + ((sizeof(rep_t)*8) - 23 - 1)))) result++;
        if (round == (1U << (23 + ((sizeof(rep_t)*8) - 23 - 1)))) result += result & 1;
    }
    result += (rep_t)(exponent + (((1 << ((sizeof(rep_t)*8) - 23 - 1)) - 1) >> 1)) << 23;
