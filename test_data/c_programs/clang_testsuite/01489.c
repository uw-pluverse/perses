...
uint64_t __fixtfdi(long double input)
{
 ...
 const uint32_t absHighWord = (uint32_t)(hibits.x >> 32) & 0x7fffffffU;
 const uint32_t absHighWordMinusOne = absHighWord - 0x3ff00000U;
 if (0x03f00000U > absHighWordMinusOne)
 {
  const int unbiasedHeadExponent = absHighWordMinusOne >> 20;
  int64_t result = hibits.x & 0x000fffffffffffffL;
  result |= 0x0010000000000000L;
  result <<= 10;
 ...
 if (1.0 > __builtin_fabs((x.s.hi)))
  return 0L;
 if (x.s.hi == -0x1.0p63) {
  int64_t result = (-9223372036854775807L -1);
  if (0.0 < x.s.lo)
  {
   const doublebits lobits = { .d = x.s.lo };
   int64_t tailMantissa = lobits.x & 0x000fffffffffffffL;
   tailMantissa |= 0x0010000000000000L;
   tailMantissa = (tailMantissa ^ -1L) + 1L;
   const int biasedTailExponent = (int)(lobits.x >> 52) & 0x7ff;
