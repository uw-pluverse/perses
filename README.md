...
- Download a prebuilt release JAR file from our [release page](https://github.com/perses-project/perses/releases),
  ...
  ```bash
  wget https://github.com/uw-pluverse/perses/releases/download/v2.7/perses_deploy.jar
...
[Classical Perses Reducer Control]  Options:
    --default-list-minimizer-for-kleene
      ...
      Default: DFS
      Possible Values: [PRISTINE_DDMIN, PERSES_VARIANT_OF_PRISTINE, DFS, BFS, CDD, WEIGHTED_DFS, WEIGHTED_BFS, PROBDD, WDD, WPROBDD, WINDOWED_SLICER, LOCAL_EXHAUSTIVE_PATTERN_ENUMERATION, ONE_BY_ONE, ADAPTIVE_GAIN_DRIVEN, DRDD, WDRDD, PRISTINE_DRDD, PRISTINE_WDRDD, CDD_ONE_MINIMAL, LITHIUM_MINIMIZE, DFS_ONE_MINIMAL]
...
[List Minimizer Microbenchmarking]  Options:
    ...
    --list-minimizers-to-evaluate
      ...
      Default: []
    --evaluation-output
      EVALUATE: the root to write the metrics under, one directory per 
      minimizer, each holding summary.jsonl and queries.jsonl. runs.jsonl at 
      the root says what became of each.
...
[SFC (Structure Form Conversion) Reducer Control]  Options:
    --enable-sfc
      Enable the SFC reducers (structure form conversion, to produce smaller 
      reduction output at the cost of more property tests).
      Default: false
    --sfc-fixpoint
      Enable fixpoint mode for running the SFC reducers.
      Default: true
    --sfc-subtree-token-count-limit
      Skip a subtree whose token count exceeds this limit: the number of 
      structures of a subtree grows with the number of its reusable pieces.
      Default: 64
    --sfc-candidate-limit
      The maximum number of candidate structures to test for a single subtree.
      Default: 64
