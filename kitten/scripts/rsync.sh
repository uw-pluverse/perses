...
readonly EXCLUDED_FOLDERS=(".ijwb" \
                           "temp*" \
                           "default*" \
                           "test_data" \
                           )
...
rsync ${OPTIONS} ${EXCLUDED_FLAGS} "$(pwd)/perses-fuzzer" "${HOST}:${REMOTE_FOLDER}"
...
if "${PULL}" ; then
  ...
  rsync ${OPTIONS} ${EXCLUDED_FLAGS} --exclude '.git' "${HOST}:${REMOTE_FOLDER}" "$(pwd)/perses-fuzzer"
fi
