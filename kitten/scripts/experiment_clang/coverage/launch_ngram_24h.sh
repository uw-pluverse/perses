...
java -Xmx32G -Xms16G -jar bazel-bin/kitten/src/org/perses/fuzzer/kira_deploy.jar \
  --testing-config "${SCRIPT_DIR}/../clang-ngram.yaml" \
  --threads 1 \
  --timeout 86400 \
  --shuffle-seed false \
  --verbosity "INFO" \
  --finding-folder "${FINDING_FOLDER}" \
  --coverage-info "${PWD}/kitten/temp_compilers_source/clang_ngram/llvm-build-trunk" \
  --compiler-source "${PWD}/kitten/temp_compilers_source/clang_ngram/llvm-project" \
  --coverage-interval 600 \
  --coverage-save "${PWD}/kitten/temp_compilers_source/clang_ngram/coverage" \
  --language-model "N_GRAM_MODEL" \
  --seed-limit 20000
