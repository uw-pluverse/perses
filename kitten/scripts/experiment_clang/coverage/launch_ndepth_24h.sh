...
java -Xmx32G -Xms16G -jar bazel-bin/kitten/src/org/perses/fuzzer/kira_deploy.jar \
  ...
  --coverage-info "${PWD}/kitten/temp_compilers_source/clang_ndepth/llvm-build-trunk" \
  --compiler-source "${PWD}/kitten/temp_compilers_source/clang_ndepth/llvm-project" \
  --coverage-interval 600 \
  --coverage-save "${PWD}/kitten/temp_compilers_source/clang_ndepth/coverage" \
