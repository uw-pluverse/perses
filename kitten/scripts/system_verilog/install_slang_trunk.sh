...
cmake -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX="${INSTALLROOT}" -DSLANG_SANITIZERS="address,undefined"  "${SRC_FOLDER}" 
make -j "${NCPUS}" 
make install

cd "${HOME_USR_BIN}"
ln -s "${INSTALLROOT}/bin/slang" "slang-trunk"
