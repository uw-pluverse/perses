...
if ! grep --quiet --fixed-strings "432 | is_rust_object || i > 3 { break ; } let msg_segv = \"clang: error: unable to execute command: Segmentation fault: 11\" ; let msg_bus = \"cla..." "${OUTPUT}" ; then
  exit 1
fi
exit 0