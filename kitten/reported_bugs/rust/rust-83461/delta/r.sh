...
if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'call to rustc_peek should be preceded by assignment to temporary holding its argument'" "${OUTPUT}" ; then
  exit 1
fi
