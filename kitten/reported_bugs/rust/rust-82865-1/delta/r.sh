...
if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'compiler/rustc_resolve/src/lib.rs:1936:21: undetermined resolution during main resolution pass'" "${OUTPUT}" ; then
  exit 1
fi
