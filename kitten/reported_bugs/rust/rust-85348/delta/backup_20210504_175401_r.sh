...
if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'compiler/rustc_resolve/src/late.rs:1776:18: unexpected resolution for an identifier in pattern: Def(ConstParam" "${OUTPUT}" ; then
  exit 1
fi
