...
if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'no errors encountered even though \`delay_span_bug\` issued', compiler/rustc_errors/src/lib.rs:961:13" "${OUTPUT}" ; then
  exit 1
fi
