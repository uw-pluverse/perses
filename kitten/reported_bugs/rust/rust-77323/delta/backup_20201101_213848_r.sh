...
if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'cannot use the \`#[rustc_if_this_changed]\` or \`#[rustc_then_this_would_need]\` annotations without supplying \`-Z query-dep-graph\`', compiler/rustc_incremental/src/assert_dep_graph.rs:77:13" "${OUTPUT}" ; then
  exit 1
fi
...
if ! grep --quiet --fixed-strings "left: \`LLVMing\`" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "right: \`Codegenning\`'" "${OUTPUT}" ; then
  exit 1
fi
exit 0