...
if ! grep --quiet --fixed-strings "error: internal compiler error: the following error was constructed but not emitted" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "thread 'rustc' panicked at 'explicit panic'" "${OUTPUT}" ; then
  exit 1
fi
