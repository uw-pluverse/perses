...
if ! rustfmt +1.49 --edition 2018 mutant.rs ; then
  exit 1
fi	
...
if ! grep --quiet --fixed-strings "error: internal compiler error: compiler/rustc_metadata/src/rmeta/decoder.rs" "${OUTPUT}" ; then
  exit 1
fi
