...
if ! grep --quiet --fixed-strings "at dotty.tools.dotc.typer.Typer.typeSelectOnTerm\$1" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "at dotty.tools.dotc.typer.Typer.typedSelect" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "at dotty.tools.dotc.typer.Typer.typedNamed\$1" "${OUTPUT}" ; then
  exit 1
fi
exit 0