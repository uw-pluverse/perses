...
if ! grep --quiet --fixed-strings "Exception in thread \"main\" java.lang.RuntimeException: Unexpected New(...collection/TypeTree[TypeRef(ThisType(TypeRef(NoPrefix,module class scala)),module class collection)]) reached GenBCode." "${OUTPUT}" ; then
  exit 1
fi
