...
if ! grep --quiet --fixed-strings "const clang::ExtQualsTypeCommonBase* clang::QualType::getCommonPtr() const: Assertion \`!isNull() && \"Cannot retrieve a NULL type pointer\"' failed." "${OUTPUT}" ; then
  exit 1
fi
