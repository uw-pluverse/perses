...
if ! grep --quiet --fixed-strings "static bool llvm::isa_impl_cl<To, const From*>::doit(const From*) [with To = clang::ParenExpr; From = clang::Expr]: Assertion \`Val && \"isa<> used on a null pointer\"' failed." "${OUTPUT}" ; then
  exit 1
fi
