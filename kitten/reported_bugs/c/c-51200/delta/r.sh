...
if ! grep --quiet --fixed-strings "llvm::Value* llvm::IRBuilderBase::CreatePtrDiff(llvm::Value*, llvm::Value*, const llvm::Twine&): Assertion \`LHS->getType() == RHS->getType() && \"Pointer subtraction operand types must match!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
