...
if ! grep --quiet --fixed-strings "void {anonymous}::FPS::handleTwoArgFP(llvm::MachineBasicBlock::iterator&): Assertion \`UpdatedSlot < StackTop && Dest < 7' failed." "${OUTPUT}" ; then
  exit 1
fi
