...
if ! grep --quiet --fixed-strings "bool clang::Type::isConstantSizeType() const: Assertion \`!isIncompleteType() && \"This doesn't make sense for incomplete types\"' failed." "${OUTPUT}" ; then
  exit 1
fi
