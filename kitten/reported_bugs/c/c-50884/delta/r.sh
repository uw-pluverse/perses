...
if ! grep --quiet --fixed-strings "bool EvaluateAsBooleanCondition(const clang::Expr*, bool&, {anonymous}::EvalInfo&): Assertion \`E->isPRValue() && \"missing lvalue-to-rvalue conv in bool condition\"' failed." "${OUTPUT}" ; then
  exit 1
fi
