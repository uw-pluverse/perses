...
if ! grep --quiet --fixed-strings "void llvm::StoreInst::AssertOK(): Assertion \`cast<PointerType>(getOperand(1)->getType()) ->isOpaqueOrPointeeTypeMatches(getOperand(0)->getType()) && \"Ptr must be a pointer to Val type!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
