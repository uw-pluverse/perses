...
if ! grep --quiet --fixed-strings "internal compiler error: in extract_ops_from_tree, at gimple-expr.c" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "extract_ops_from_tree(tree_node*, tree_code*, tree_node**, tree_node**, tree_node**)" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "gimple_build_assign(tree_node*, tree_node*)" "${OUTPUT}" ; then
  exit 1
fi
