...
if ! grep --quiet --fixed-strings "clang::CodeGen::LValue clang::CodeGen::CodeGenFunction::EmitDeclRefLValue(const clang::DeclRefExpr*): Assertion \`E->isNonOdrUse() != NOUR_Unevaluated && \"should not emit an unevaluated operand\"' failed." "${OUTPUT}" ; then
  exit 1
fi
