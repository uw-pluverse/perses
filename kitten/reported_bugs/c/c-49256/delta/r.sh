...
if ! grep --quiet --fixed-strings "static llvm::IntegerType* llvm::IntegerType::get(llvm::LLVMContext&, unsigned int): Assertion \`NumBits >= MIN_INT_BITS && \"bitwidth too small\"' failed." "${OUTPUT}" ; then
  exit 1
fi
