...
if ! grep --quiet --fixed-strings "clang::CodeGen::ConstantAddress clang::CodeGen::CodeGenModule::GetWeakRefReference(const clang::ValueDecl*): Assertion \`AA && \"No alias?\"' failed." "${OUTPUT}" ; then
  exit 1
fi
