...
if ! grep --quiet --fixed-strings "void checkAttributesAfterMerging(clang::Sema&, clang::NamedDecl&): Assertion \`VD->isThisDeclarationADefinition() && !VD->isExternallyVisible() && \"Broken AliasAttr handled late!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
