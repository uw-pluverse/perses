...
if ! grep --quiet --fixed-strings "internal compiler error: in expand_asm_stmt, at cfgexpand.c" "${OUTPUT}" ; then
  exit 1
fi

if ! grep --quiet --fixed-strings "expand_asm_stmt" "${OUTPUT}" ; then
  exit 1
fi
