...
if ! grep --quiet --fixed-strings "bool {anonymous}::ConstantAggregateBuilder::add(llvm::Constant*, clang::CharUnits, bool): Assertion \`(FirstElemToReplace == LastElemToReplace || AllowOverwrite) && \"unexpectedly overwriting field\"' failed." "${OUTPUT}" ; then
  exit 1
fi
