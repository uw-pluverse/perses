...
if ! grep --quiet --fixed-strings "clang::CodeGen::Address clang::CodeGen::RValue::getAggregateAddress() const: Assertion \`isAggregate() && \"Not an aggregate!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
