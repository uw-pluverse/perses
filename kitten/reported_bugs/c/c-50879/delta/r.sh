...
if ! grep --quiet --fixed-strings "static clang::OverloadExpr::FindResult clang::OverloadExpr::find(clang::Expr*): Assertion \`E->getType()->isSpecificBuiltinType(BuiltinType::Overload)' failed." "${OUTPUT}" ; then
  exit 1
fi
