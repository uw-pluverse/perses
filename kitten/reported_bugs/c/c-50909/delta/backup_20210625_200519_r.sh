...
if ! grep --quiet --fixed-strings "void {anonymous}::CGRecordLowering::clipTailPadding(): Assertion \`Prior->FD->hasAttr<NoUniqueAddressAttr>() && \"should not have reused this field's tail padding\"' failed." "${OUTPUT}" ; then
  exit 1
fi
