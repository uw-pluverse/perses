...
if ! grep --quiet --fixed-strings "void llvm::Value::doRAUW(llvm::Value*, llvm::Value::ReplaceMetadataUses): Assertion \`New->getType() == getType() && \"replaceAllUses of value with new value of different type!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
