...
if ! grep --quiet --fixed-strings "unsigned int clang::GenericSelectionExpr::getResultIndex() const: Assertion \`!isResultDependent() && \"Generic selection is result-dependent but getResultIndex called!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
