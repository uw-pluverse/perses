...
if ! grep --quiet --fixed-strings "const clang::ASTRecordLayout& clang::ASTContext::getASTRecordLayout(const clang::RecordDecl*) const: Assertion \`!D->isInvalidDecl() && \"Cannot get layout of invalid decl!\"' failed." "${OUTPUT}" ; then
  exit 1
fi
