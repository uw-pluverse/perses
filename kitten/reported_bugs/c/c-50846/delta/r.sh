...
if ! grep --quiet --fixed-strings "llvm::APInt llvm::APInt::operator*(const llvm::APInt&) const: Assertion \`BitWidth == RHS.BitWidth && \"Bit widths must be the same\"' failed." "${OUTPUT}" ; then
  exit 1
fi
