...
if ! grep --quiet --fixed-strings "bool clang::Sema::CheckForConstantInitializer(clang::Expr*, clang::QualType): Assertion \`Init->containsErrors() && \"Dependent code should only occur in error-recovery path.\"' failed." "${OUTPUT}" ; then
  exit 1
fi
