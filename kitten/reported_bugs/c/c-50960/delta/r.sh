...
if ! grep --quiet --fixed-strings "double llvm::APFloat::convertToDouble() const: Assertion \`getSemantics().isRepresentableBy(semIEEEdouble) && \"Float semantics is not representable by IEEEdouble\"' failed." "${OUTPUT}" ; then
  exit 1
fi
