...
abstract class AbstractSparTreeGenerator(
  ...
  fun generateFromStartSymbol(): SparTree? {
    ...
    if (
      ...
      ruleGenerationInfo
        .getMinDepth(
          grammarHierarchy
            .getRuleHierarchyEntryWithNameOrThrow(
  ...
  fun generateParserRuleSparTreeNode(ruleNameHandle: RuleNameHandle): ParserRuleSparTreeNode? {
    val ruleDef =
      grammarHierarchy
        .getRuleHierarchyEntryWithNameOrThrow(ruleNameHandle.ruleName)
        .ruleDef
  ...
  fun generateRandomNonEOFToken(random: Random): Token {
    val lexerRules = parserFacade.antlrGrammar.lexerRules
    var selectedLexerRules: PersesLexerRuleAst
    do {
      selectedLexerRules = random.sample(lexerRules)
    } while (selectedLexerRules.body.sourceCode == "EOF")
    return generateAntlrToken(selectedLexerRules.body)
  }
  ...
  @Synchronized
  private fun generateAntlrToken(ruleBody: AbstractPersesRuleElement): Token {
        ...
        is PersesNotAst -> {
          when (val subRuleBody = ruleBody.getChild(0)) {
            ...
            is PersesTerminalAst -> {
              ...
              if (text == "EOF") {
                // Better to directly use the returned antlr token, but not recreating the token.
                generateRandomNonEOFToken(random).text
              } else {
  ...
  fun updateCorpus(tree: SparTree) {
    for (lexerNode in tree.remainingLexerRuleNodes) {
      val tokenType: TokenType = lexerNode.token.tokenType
  ...
  companion object {
    ...
    val ANTLRLiteralEscapedCharValue =
      listOf(
        'n' to '\n',
        'r' to '\r',
        't' to '\t',
    ...
    fun getTokenList(node: AbstractSparTreeNode?): ImmutableList<AbstractPersesToken> {
      if (node == null) {
      ...
      node.preOrderVisit {
        if (it.isTokenNode()) {
          builder.add(it.asLexerRule().token.asAntlrToken())
        }
